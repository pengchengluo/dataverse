...
public class FeaturedDataverseServiceBean {
    ...
    public List<Dataverse> findByDataverseIdQuick(Long dataverseId) {
         ...
         try {
             ...
             if(dataverseLocale.isLocaleZh()){
                 searchResults = em.createNativeQuery("SELECT d.id, d.alias, d.name_zh FROM dataverse d, DataverseFeaturedDataverse f WHERE f.featureddataverse_id = d.id AND f.dataverse_id = "+dataverseId+" order by f.displayOrder").getResultList();
             }else{
         ...
         for (Object[] result : searchResults) {
            ...
            if (result[2] != null) {
                if(dataverseLocale.isLocaleZh()){
                    dataverse.setNameZh((String)result[2]);
                }else{
